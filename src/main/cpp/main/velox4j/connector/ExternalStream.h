...
public:
  ...
  virtual std::optional<facebook::velox::RowVectorPtr>
  read(facebook::velox::ContinueFuture &future) = 0;
};

class ExternalStreamConnectorSplit
    : public facebook::velox::connector::ConnectorSplit {
public:
  ExternalStreamConnectorSplit(const std::string &connectorId,
                               ObjectHandle esId);
  ...
  static std::shared_ptr<ExternalStreamConnectorSplit>
  create(const folly::dynamic &obj, void *context);
...
public:
  ...
  std::string toString() const override { return "ExternalStreamTableHandle"; }

  folly::dynamic serialize() const override;

  static void registerSerDe();

  static facebook::velox::connector::ConnectorTableHandlePtr
  create(const folly::dynamic &obj, void *context);
};
...
public:
  ...
  std::shared_ptr<facebook::velox::connector::Connector> newConnector(
      ...
      folly::Executor *ioExecutor, folly::Executor *cpuExecutor) override;
};
