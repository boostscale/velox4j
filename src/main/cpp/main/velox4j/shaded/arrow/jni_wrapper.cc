...
jfieldID
GetFieldID(JNIEnv* env, jclass this_class, const char* name, const char* sig) {
...
void release_exported(T* base) {
  ...
  try {
    ...
    env->CallObjectMethod(
        private_data->j_private_data_, kPrivateDataCloseMethod);
...
void TryCopyLastError(JNIEnv* env, InnerPrivateData* private_data) {
  jobject error_data = env->GetObjectField(
      private_data->j_private_data_, kPrivateDataLastErrorField);
  if (!error_data) {
...
void TryHandleUncaughtException(
    JNIEnv* env,
    InnerPrivateData* private_data,
    jthrowable exc) {
  jstring message = reinterpret_cast<jstring>(
      env->CallObjectMethod(exc, kObjectToStringMethod));
  if (!message) {
    private_data->last_error_.clear();
    return;
  }
...
int ArrowArrayStreamGetSchema(ArrowArrayStream* stream, ArrowSchema* out) {
  ...
  const int err_code = env->CallIntMethod(
      private_data->j_private_data_, kPrivateDataGetSchemaMethod, out_addr);
...
int ArrowArrayStreamGetNext(ArrowArrayStream* stream, ArrowArray* out) {
  ...
  const jlong out_addr = static_cast<jlong>(reinterpret_cast<uintptr_t>(out));
  const int err_code = env->CallIntMethod(
      private_data->j_private_data_, kPrivateDataGetNextMethod, out_addr);
  if (jthrowable exc = env->ExceptionOccurred()) {
    TryHandleUncaughtException(env, private_data, exc);
    env->ExceptionClear();
    return EIO;
...
const char* ArrowArrayStreamGetLastError(ArrowArrayStream* stream) {
  ...
  if (private_data->last_error_.empty())
    return nullptr;
...
jint JNI_OnLoad_shaded_C(JavaVM* vm, void* reserved) {
  JNIEnv* env;
  ...
  kCDataExceptionConstructor = GetMethodID(
      env, kCDataExceptionClass, "<init>", "(ILjava/lang/String;)V");

  return JNI_VERSION;
  JNI_METHOD_END(JNI_ERR)
}

void JNI_OnUnload_shaded_C(JavaVM* vm, void* reserved) {
  JNIEnv* env;
