...
template <typename T>
T getConfig(const std::unordered_map<std::string, std::string> &configs,
...
public:
      ...
      : MemoryArbitrator(config), listener_(listener),
        memoryPoolInitialCapacity_(velox::config::toCapacity(
            getConfig<std::string>(
                config.extraConfigs, kMemoryPoolInitialCapacity,
        ...
        memoryPoolTransferCapacity_(velox::config::toCapacity(
            getConfig<std::string>(
                config.extraConfigs, kMemoryPoolTransferCapacity,
        ...
        memoryReclaimMaxWaitMs_(
            std::chrono::duration_cast<std::chrono::milliseconds>(
                velox::config::toDuration(getConfig<std::string>(
                    config.extraConfigs, kMemoryReclaimMaxWaitMs,
  ...
  void
  addPool(const std::shared_ptr<velox::memory::MemoryPool> &pool) override {
  ...
  void growCapacity(velox::memory::MemoryPool *pool,
    ...
    VELOX_CHECK(pool->root() == candidate,
                "Illegal state in ListenableArbitrator");

    growCapacityInternal(pool->root(), targetBytes);
  }

  uint64_t shrinkCapacity(uint64_t targetBytes, bool allowSpill,
    ...
    pool->reclaim(targetBytes, memoryReclaimMaxWaitMs_,
                  status); // ignore the output
    return shrinkCapacityInternal(pool, 0);
  }

  uint64_t shrinkCapacity(velox::memory::MemoryPool *pool,
                          uint64_t targetBytes) override {
...
private:
  void growCapacityInternal(velox::memory::MemoryPool *pool, uint64_t bytes) {
    ...
    VELOX_CHECK(ret, "{} failed to grow {} bytes, current state {}",
                pool->name(), velox::succinctBytes(bytes), pool->toString());
  }

  uint64_t shrinkCapacityInternal(velox::memory::MemoryPool *pool,
                                  uint64_t bytes) {
...
MemoryManager::getVeloxPool(const std::string &name,
                            const velox::memory::MemoryPool::Kind &kind) {
  if (veloxPoolRefs_.count(name) > 0) {
    const auto &pool = veloxPoolRefs_[name];
    VELOX_CHECK_EQ(pool->kind(), kind,
                   "Pool [" + name +
                       "] was already created but with different kind");
    return pool.get();
  }
  switch (kind) {
