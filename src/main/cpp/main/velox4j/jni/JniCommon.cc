...
void checkException(JNIEnv *env) {
  if (env->ExceptionCheck()) {
    ...
    std::string description =
        jStringToCString(env, (jstring)env->CallStaticObjectMethod(
    ...
    VELOX_FAIL("Error during calling Java code from native code: " +
               description);
  }
}
...
template <typename T> T *jniCastOrThrow(jlong handle) {
  ...
  VELOX_CHECK(instance != nullptr,
              "FATAL: resource instance should not be null.");
  return instance;
}
